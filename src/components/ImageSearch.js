...
const ImageSearch = ({ onSelect }) => {
  ...
  return (
    <div className="p-4">
      ...
      <button 
        onClick={fetchImages} 
        className="bg-blue-300 hover:bg-blue-600 hover:text-white text-black  p-2 rounded ml-2"
      ...
      {images.length === 0 && !loading && !error && (
        <p className='text-white'> Enter your search query here...</p> // Empty state message
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4">
        {images.map((image) => (
          <div key={image.id} className="cursor-pointer" onClick={() => onSelect(image.urls.regular)}>
            ...
            <button className="bg-green-500 text-white p-2 w-full rounded mt-2">Add Captions</button>
          </div>
        ))}
      </div>
    </div>
